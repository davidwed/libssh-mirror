...
#ifdef WITH_PCAP
static void set_pcap(struct session_data_st *sdata, ssh_session session,
        char *pcap_file)
{
    int rc = 0;
...
static int authenticate_kbdint(ssh_session session, ssh_message message,
        void *userdata)
{
    int rc = 0;
    int count;
    ...
    case 1:
        ...
        if (strncmp(answer, sdata->username, expected_len)) {
            goto failed;
        }
        ...
        if (strncmp(answer, sdata->password, expected_len)) {
            goto failed;
        }

        /* Username and password matched. Ask for a retype. */
        ssh_message_auth_interactive_request(message, name, instruction, 1,
                retype_prompt, retype_echo);
...
static int authenticate_callback(ssh_session session, ssh_message message,
        void *userdata)
{
    struct session_data_st *sdata = (struct session_data_st *)userdata;
    int rc;
...
static void handle_kbdint_session_cb(ssh_event event, ssh_session session,
