...
int auth_pubkey_cb(ssh_session session, const char *user,
...
void channel_signal_cb (ssh_session session, ssh_channel channel,
        ...
        void *userdata)
{
    ...
    if (rc == 0) {
        rc = channel_exec_request_cb(session, channel, SFTP_SERVER_PATH,
                userdata);
        goto end;
    }
