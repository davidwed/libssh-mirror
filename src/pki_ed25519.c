...
int
pki_pubkey_build_ed25519(ssh_key key, ssh_string pubkey)
{
...
int
pki_privkey_build_ed25519(ssh_key key, ssh_string pubkey, ssh_string privkey)
{
    if (ssh_string_len(pubkey) != ED25519_KEY_LEN ||
 ...
 */
int
pki_ed25519_key_dup(ssh_key new_key, const ssh_key key)
{
 ...
 */
int
pki_ed25519_public_key_to_blob(ssh_buffer buffer, ssh_key key)
{
    int rc;
 ...
 */
int
pki_ed25519_private_key_to_blob(ssh_buffer buffer, const ssh_key privkey)
{
    int rc;
